...
declare var $: any;
@Component({
  selector: 'app-assign-volunteer-clinic',
  templateUrl: './assign-volunteer-clinic.page.html',
  styleUrls: ['./assign-volunteer-clinic.page.scss'],
})
export class AssignVolunteerClinicPage implements OnInit {
  ...
  ngOnInit() {
    var user_id = localStorage.getItem("user_id");
      if (user_id === null) {
        ...
        formData.append("key", "get_volunteer_all_asc");
    ...
    addAssignDoctorForm(form){
      var formData: any = new FormData();
      formData.append("key", "assign_volunteer_to_clinic");
      formData.append("clinic", form.value.clinic_name.id);
      formData.append("volunteer", this.selectedArray);
      this.http.post(this.baseURI+'manage-clinic-volunteer-map.php',formData).subscribe((res : any) => {
        if(res['status']==1){
          $("#p_form")[0].reset();
          this.router.navigate(['/assign-volunteer-to-clinic']);
          this.toastMessage("Volunteer assigned succesfully.");
          }else if(res['status']==2){
            this.toastMessage("Volunteer already assigned to this clinic.");
          }else{
