...
export class PatientHistoryPage implements OnInit {
      ...
      if (user_id === null) {
        ...
        this.http.post(this.baseURI+'manage-patient-history.php',formData).subscribe((res : any) => {
          if(res!=''){
              //console.log(res);
              this.history = res;
              this.btnTextName = 'Update History';
            }else{
              //console.log('hello');
              this.btnTextName = 'Add History';
            }
  ...
  patientHistoryForm(form){
    ...
    this.http.post(this.baseURI+'manage-patient-history.php',formData).subscribe((res : any) => {
      if(res['status'] == 1){
        if(res['msg'] == 'add'){
          this.toastMessage("Patient history added successfully.");
          window.history.back();
        }else{
          this.toastMessage("Patient history updated successfully.");
          window.history.back();
        }
