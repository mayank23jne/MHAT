...
@Component({
  selector: 'app-add-supervisor-feedback',
  templateUrl: './add-supervisor-feedback.page.html',
  styleUrls: ['./add-supervisor-feedback.page.scss'],
})
export class AddSupervisorFeedbackPage implements OnInit {
  ...
  patientFeedbackForm(form){
    ...
    this.http.post(this.baseURI+'manage-feedback.php',formData).subscribe((res : any) => {
      if(res['status'] == 1){
          this.toastMessage("Feedback added successfully.");
          window.history.back();          
        }else{
